<?php
		...
		$model = Rooms::model()->findByAttributes(array('id_room'=>1));
		...
		//array('label'=>'Eliminar sala', 'url'=>'#', 'linkOptions'=>array( 'submit'=>array( '/rooms/delete', 'id'=>1), 'confirm'=>'Tem a certeza que pretende remover esta sala?' )),
		),
		)
		);
		?>
	...
	&nbsp;&nbsp;&nbsp;<?php echo $model->description; ?><table width="100%"><tr><td width="50%">
            <br><br>
            <h5>Salas Portuguese</h5>
            <?php 
            $dataProvider = IndexingCreators::getObjectWorkRecords_NationalitiesCreator('Portuguese');
            ...
            <h5>Salas Haitian</h5>
            <?php 
            $dataProvider = IndexingCreators::getObjectWorkRecords_NationalitiesCreator('Haitian');
            $dataProvider->pagination->pageSize=2;
...
<!-- Sistema de navegação --><br/><br/><hr/>
		...
		WHERE Exhibitionsid_exhibition=1 and Roomsid_room=1)'
		);
