...
<!-- Sistema de navegação -->
		<?php
			$prev_room = Rooms::model()->findAllBySql('SELECT R.*
					...
					WHERE Exhibitionsid_exhibition=1 and Roomsid_room=4)'
			);
			...
			if (!empty($prev_room)) {
				...
				if (isset($matches[1])){
					...
					echo CHtml::button("Anterior - ".$prev_room[0]->name, array('/site/sala', 'view'=>$matches[1]));
				}
		...
		<?php
			 if (!empty($next_room)) {
			 	...
			 	if (isset($matches[1])){
			 		...
			 		echo CHtml::button("Seguinte - ".$next_room[0]->name, array('/site/sala', 'view'=>$matches[1]));
				}
