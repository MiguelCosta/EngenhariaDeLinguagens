<?php
		...
		$model = Rooms::model()->findByAttributes(array('id_room'=>4));
		...
		//array('label'=>'Eliminar sala', 'url'=>'#', 'linkOptions'=>array( 'submit'=>array( '/rooms/delete', 'id'=>4), 'confirm'=>'Tem a certeza que pretende remover esta sala?' )),
		),
		)
		);
		?>
	...
	&nbsp;&nbsp;&nbsp;<?php echo $model->description; ?><table width="100%"><tr><td width="50%">
            <br><br>
            <h5>Peças de Eduardo de Moura</h5>
            <?php 
            $dataProvider = IndexingCreators::getObjectWorkRecords_NameCreator('Eduardo de Moura');
...
<!-- Sistema de navegação --><br/><br/><hr/>
		...
		WHERE Exhibitionsid_exhibition=3 and Roomsid_room=4)'
		);
