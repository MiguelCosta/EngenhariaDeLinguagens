<?php
		...
		$model = Rooms::model()->findByAttributes(array('id_room'=>2));
		...
		//array('label'=>'Eliminar sala', 'url'=>'#', 'linkOptions'=>array( 'submit'=>array( '/rooms/delete', 'id'=>2), 'confirm'=>'Tem a certeza que pretende remover esta sala?' )),
		),
		)
		);
		?>
	...
	&nbsp;&nbsp;&nbsp;<?php echo $model->description; ?><table width="100%"><tr><td width="50%">
            <br><br>
            <h5>Peças de Gomes Ferreira</h5>
            <?php 
            $dataProvider = IndexingCreators::getObjectWorkRecords_NameCreator('Gomes Ferreira');
            ...
            <h5>Peças de Lisboa</h5>
            <?php 
            $dataProvider = LocationsName::getObjectWorkRecords_LocationsName('Lisboa');
...
<!-- Sistema de navegação --><br/><br/><hr/>
		...
		WHERE Exhibitionsid_exhibition=3 and Roomsid_room=2)'
		);
